...
@pytest.mark.parametrize('show_name,ep_name,pattern,expected', [
    ...
    (
        "C'est un exemple",
        "L'\xe9pisode de janvier",
        '%S.N.S%0SE%0E.%E.N',
        'C.est.un.exemple.S01E01.L.\xe9pisode.de.janvier',
    ),
    (
        'Serie exemple',
        "Et si l\u2019exemple existait ?",
        '%S.N.S%0SE%0E.%E.N',
        'Serie.exemple.S01E01.Et.si.l.exemple.existait',
    ),
    # Plain tokens keep the apostrophe; colon/question use sanitize_filename.
    (
        'Example show : Special subtitle',
        'Titre normal',
        '%SN - S%0SE%0E - %EN',
        'Example show - Special subtitle - S01E01 - Titre normal',
    ),
])
...
def test_sanitize_scene_name_unchanged_for_search():
    ...
    assert sanitize_scene_name("C'est un exemple") == 'Cest.un.exemple'
    assert sanitize_scene_name("L'\xe9pisode") == 'L\xe9pisode'
